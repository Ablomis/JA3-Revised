...
function MoveItem(args)
  ...
  if is_upgrade then
    local weapon_obj = FindWeaponUpgradeTarget(item_at_dest, item)
    if not weapon_obj then
      return "invalid upgrade target"
    end
    if Sync() then
      return sync_err
    end
    ...
    weapon_obj:SetWeaponComponent(item.Slot, item.Name)
    if src_container then
     src_container:RemoveItem(src_container_slot_name, item, "no_update")
    end
    DoneObject(item)
